...
sserver <- function(input, output, session) {
  ...
  result <- eventReactive(input$search, {
    ...
    # Validate year input
    year_found <- suppressWarnings(as.numeric(input$year))
    if (is.na(year_found) || year_found < 1800 || year_found > as.numeric(format(Sys.Date(), "%Y"))) {
      return("Please enter a valid year between 1800 and the current year.")
    }
    ...
    tryCatch({
      ...
      if (length(match) == 0) {
        ...
        new_record <- data.frame(
          ...
          YearFound = as.integer(year_found),
          MarineRegion = region,
          stringsAsFactors = FALSE
        )
        ...
        return(paste(
          ...
          "Year found:", as.integer(year_found), "\n",
